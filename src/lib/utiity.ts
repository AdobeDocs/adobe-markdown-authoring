...
const expressionToReplacementMap: IExpressionReplacementPair[] = [
  {
    expression: /\u201c/gm /* double left quote               “ */,
    replacement: '"',
  },
  {
    expression: /\u201d/gm /* double right quote              ” */,
    replacement: '"',
  },
  {
    expression: /\u2018/gm /* single left quote               ‘ */,
    replacement: "'",
  },
  {
    expression: /\u2019/gm /* single right quote              ’ */,
    replacement: "'",
  },
  {
    expression: /\u00A9/gm /* copyright character             © */,
    replacement: "&copy;",
  },
  {
    expression: /\u2122/gm /* trademark character             ™ */,
    replacement: "&trade;",
  },
  {
    expression: /\u00AE/gm /* registered trademark character  ® */,
    replacement: "&reg;",
  },
  {
    expression: /\u20AC/gm /* euro character                  € */,
    replacement: "&euro;",
  },
  {
    expression: /\u2022/gm /* bullet character                • */,
    replacement: "*",
  },
  // Superscript
  {
    expression: /\u2070/gm /* 0 superscript character         ⁰ */,
    replacement: "<sup>0</sup>",
  },
  {
    expression: /\u00B9/gm /* 1 superscript character         ⁴ */,
    replacement: "<sup>1</sup>",
  },
  {
    expression: /\u00B2/gm /* 2 superscript character         ⁴ */,
    replacement: "<sup>2</sup>",
  },
  {
    expression: /\u00B3/gm /* 3 superscript character         ⁴ */,
    replacement: "<sup>3</sup>",
  },
  {
    expression: /\u2074/gm /* 4 superscript character         ⁴ */,
    replacement: "<sup>4</sup>",
  },
  {
    expression: /\u2075/gm /* 5 superscript character         ⁵ */,
    replacement: "<sup>5</sup>",
  },
  {
    expression: /\u2076/gm /* 6 superscript character         ⁶ */,
    replacement: "<sup>6</sup>",
  },
  {
    expression: /\u2077/gm /* 7 superscript character         ⁷ */,
    replacement: "<sup>7</sup>",
  },
  {
    expression: /\u2078/gm /* 8 superscript character         ⁸ */,
    replacement: "<sup>8</sup>",
  },
  {
    expression: /\u2079/gm /* 9 superscript character         ⁹ */,
    replacement: "<sup>9</sup>",
  },
  // Subscript
  {
    expression: /\u2080/gm /* 0 subscript character           ₀ */,
    replacement: "<sub>0</sub>",
  },
  {
    expression: /\u2081/gm /* 1 subscript character           ₁ */,
    replacement: "<sub>1</sub>",
  },
  {
    expression: /\u2082/gm /* 2 subscript character           ₂ */,
    replacement: "<sub>2</sub>",
  },
  {
    expression: /\u2083/gm /* 3 subscript character           ₃ */,
    replacement: "<sub>3</sub>",
  },
  {
    expression: /\u2084/gm /* 4 subscript character           ₄ */,
    replacement: "<sub>4</sub>",
  },
  {
    expression: /\u2085/gm /* 5 subscript character           ₅ */,
    replacement: "<sub>5</sub>",
  },
  {
    expression: /\u2086/gm /* 6 subscript character           ₆ */,
    replacement: "<sub>6</sub>",
  },
  {
    expression: /\u2087/gm /* 7 subscript character           ₇ */,
    replacement: "<sub>7</sub>",
  },
  {
    expression: /\u2088/gm /* 8 subscript character           ₈ */,
    replacement: "<sub>8</sub>",
  },
  {
    expression: /\u2089/gm /* 9 subscript character           ₉ */,
    replacement: "<sub>9</sub>",
  },
];
 ...
 */
export async function findAndReplaceTargetExpressions(
    ...
    if (
      ...
      if (!!content) {
        ...
        if (workspace.getConfiguration("editor").insertSpaces) {
          const tabSize =
            (workspace.getConfiguration("editor").tabSize as number) || 4;
          if (
            !expressionToReplacementMap.some(
              (pair) => pair.expression === tabExpression
            )
          ) {
...
export function findReplacements(
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result !== null && result.length) {
      const match =
        groups && result.groups ? result.groups[groups[0]] : result[0];
      if (match) {
  ...
  if (result !== null && result.length) {
    ...
    if (match && match !== value) {
      ...
      const endPosition = new Position(
        startPosition.line,
        startPosition.character + match.length
      );
      const selection = new Selection(startPosition, endPosition);
...
export function makeRelativeLink(link: string): string {
  ...
  if (!currentFile) {
    console.log(`No current editor to compute relative links.`);
    return link;
  }
  console.log(`Current editor file path is: ${currentFile}`);
  let relpath: string = link;
  if (fs.existsSync(link)) {
    relpath = relativePath(currentFile, link);
    console.log(`Resolved absolute link path ${link} .`);
  } else {
    console.log(`Attempting to resolve relative path: ${relpath}`);
    if (folders) {
      folders.forEach((folder: WorkspaceFolder) => {
        link = path.join(folder.uri.path, link);
        if (fs.existsSync(link)) {
          console.log(`Absolute path found, and exists. ${link}`);
          relpath = relativePath(currentFile, link);
          console.log(`Resolved relative path: ${relpath}`);
          if (fs.existsSync(relpath)) {
            console.log(`Resolved relative path exists. ${relpath}`);
          } else {
            console.log(`Resolved relative path does not exist. ${relpath}`);
          }
        } else {
          console.log(`Link Path ${link} does not exist.`);
        }
      });
    }
  }
  console.log(`Relative path resolved to: ${relpath}.`);
  return relpath;
}
