...
export function removeBlankLineInsideBlockQuote() {
  const markdownlintData: any = workspace
    .getConfiguration()
    .inspect(markdownlintProperty);
...
export function addFrontMatterTitle() {
  const markdownlintData: any = workspace
    .getConfiguration()
    .inspect(markdownlintProperty);
  const addFrontMatterTitleSetting =
    workspace.getConfiguration("markdown").addFrontMatterTitle;
  // preserve existing markdownlint.config settings if they exist
  if (markdownlintData.globalValue && addFrontMatterTitleSetting) {
    ...
    showStatusMessage(
      `Added front_matter_title property to Markdownlint config setting.`
    );
  }
...
export function checkMarkdownlintConfigSettings() {
  ...
  if (!customLintConfig) {
    output.appendLine(
      `[${msTimeValue}] - Unable to load markdownlint.config from package.json. No changes made.`
    );
    return;
  }

  if (configPropertyData) {
    if (configPropertyData.globalValue !== undefined) {
      const oldConfig = configPropertyData.globalValue;
      if (JSON.stringify(customLintConfig) !== JSON.stringify(oldConfig)) {
        workspace
          .getConfiguration()
        ...
        output.appendLine(
          `[${msTimeValue}] - User already has the latest markdownlint.config settings. No changes made.`
        );
      }
    } else {
      workspace
...
//   const { msTimeValue } = generateTimestamp();
//   const configProperty = "markdownlint.config";
//   const configPropertyData: any = workspace
//     .getConfiguration()
//     .inspect(configProperty);
//   // const customLintConfig = defaultMarkdownlintConfig;
